...
def _send_capi_async(website, event_name, custom_data, user_data, request_url=None, event_id=None):
    """Fire and forget CAPI event in a background thread."""
...
def trigger_backend_capi(event_name, custom_data=None, event_id=None):
    """Collect user data from current request and send CAPI event."""
...
# ═══════════════════════════════════════════════════════════════════════
# Product Page — ViewContent (server-side)
# ═══════════════════════════════════════════════════════════════════════
class GadgetflixCapiWebsiteSale(WebsiteSale):
    ...
    def product(self, product, category=None, pricelist=None, **kwargs):
        ...
        try:
            if product and product.exists():
                ...
                # Set cookie so the browser-side template script can use the same event_id
    ...
    # ═══════════════════════════════════════════════════════════════════
    # Checkout — InitiateCheckout (server-side)
    # ═══════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════
    # Address Submit — AddShippingInfo (server-side)
    # ═══════════════════════════════════════════════════════════════════
    ...
    def shop_address_submit(self, partner_id=None, address_type='billing', use_delivery_as_billing=None, callback=None, **form_data):
        res = super().shop_address_submit(
            ...
            use_delivery_as_billing=use_delivery_as_billing, callback=callback, **form_data
        )
        try:
            order = request.cart
            if order and order.amount_total > 0:
            ...
            _logger.warning("CAPI AddShippingInfo failed: %s", e)
        return res


# ═══════════════════════════════════════════════════════════════════════
# Add to Cart — AddToCart (server-side)
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# Payment Transaction — AddPaymentInfo + Lead (server-side)
# ═══════════════════════════════════════════════════════════════════════
    ...
    def shop_payment_transaction(self, order_id, access_token, **kwargs):
        ...
        try:
            ...
            if order.exists() and order.amount_total > 0:
                # AddPaymentInfo — user is submitting payment
                trigger_backend_capi('AddPaymentInfo', {
                    'value': float(order.amount_total),
                    'currency': order.currency_id.name,
                }, event_id=f"PAYMENT_{order.id}")

                # Lead — user initiated a purchase
                trigger_backend_capi('Lead', {
                    'value': float(order.amount_total),
                    'currency': order.currency_id.name,
                }, event_id=f"LEAD_{order.id}")

                # Set cookies for browser-side matching
                request.set_cookie('gf_fb_payment_eid', f"PAYMENT_{order.id}", httponly=False)
                request.set_cookie('gf_fb_lead_eid', f"LEAD_{order.id}", httponly=False)
        except Exception as e:
